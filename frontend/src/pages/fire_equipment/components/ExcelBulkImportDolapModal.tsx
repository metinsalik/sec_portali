...
export function ExcelBulkImportDolapModal({ facilityId, onComplete }: { facilityId: string, onComplete?: () => void }) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await api.post(`/fire-equipment/equipment/bulk-dolap/${facilityId}`, { equipments: cleanedData });
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" className="gap-2 border-red-200 text-red-700 hover:bg-red-50 hover:text-red-800">
          ...
          Excel'den Dolap Yükle
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Excel'den Toplu Yangın Dolabı Yükle</DialogTitle>
        </DialogHeader>
        <div className="space-y-4 py-4">
          <p className="text-xs text-muted-foreground">
            Lütfen formata uygun bir Excel dosyası seçiniz. Sütun başlıkları şu şekilde olmalıdır: 
            <strong> blok, kat, birim, mahal, firma, not, ekipman_no, kategori, alt_kategori, sorumlu_departman, hortum_tipi, hortum_uzunlugu, hortum_capi, nozzle_capi, calisma_basinci, debi, vana_tipi, imal_yili, kapak_tipi, marka</strong>
          </p>
