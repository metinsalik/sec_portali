...
export function ExcelBulkImportFlasorModal({ facilityId, onComplete }: { facilityId: string, onComplete?: () => void }) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await api.post(`/fire-equipment/equipment/bulk-flasor/${facilityId}`, { equipments: cleanedData });
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" className="gap-2 border-red-200 text-red-700 hover:bg-red-50 hover:text-red-800">
          ...
          Excel'den Flaşör Yükle
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Excel'den Flaşör Yükle</DialogTitle>
          <DialogDescription>
