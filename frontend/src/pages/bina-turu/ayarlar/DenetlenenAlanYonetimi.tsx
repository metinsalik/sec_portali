...
const DenetlenenAlanYonetimi = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const res = await api.get(`/bina-turu/ayarlar/denetlenen-alan?facilityId=${facilityId}`);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editId) {
        await api.put(`/bina-turu/ayarlar/denetlenen-alan/${editId}`, { ad });
        toast.success('Denetlenen Alan güncellendi.');
      } else {
        await api.post(`/bina-turu/ayarlar/denetlenen-alan`, { facilityId, ad });
        toast.success('Denetlenen Alan eklendi.');
      }
      setAd('');
      setEditId(null);
      fetchData();
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await api.delete(`/bina-turu/ayarlar/denetlenen-alan/${id}`);
      toast.success('Denetlenen Alan silindi.');
      fetchData();
    } catch (err) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Denetlenen Alan Yönetimi</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="flex gap-2 mb-6">
          <Input 
            placeholder="Denetlenen Alan Adı" 
            value={ad} 
...
export default DenetlenenAlanYonetimi;
