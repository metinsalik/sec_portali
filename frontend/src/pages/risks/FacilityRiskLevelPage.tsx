...
export default function FacilityRiskLevelPage() {
  ...
  const uniqueDepartments = useMemo(() => {
    const ids = Array.from(new Set(levelRisks.map((r: any) => r.departmentId).filter(Boolean))) as string[];
    return ids.map(id => ({ id, name: departmentMap[id]?.name || 'Bilinmeyen Departman' }));
  }, [levelRisks, departmentMap]);

  const uniqueResponsibles = useMemo(() => Array.from(new Set(levelRisks.map((r: any) => r.improvementResponsible).filter(Boolean))) as string[], [levelRisks]);

  const sortedRisks = useMemo(() => {
    let sortableRisks = [...levelRisks];
    if (filterStatus) {
  ...
  }, [levelRisks, sortConfig, filterStatus, filterDepartment, filterResponsible, filterInitialLevel, filterFinalLevel]);
  ...
  const pageTitle = level || 'Tüm Risk Seviyeleri';
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="sm" onClick={() => navigate(`/risks/facility/${facilityId}`)} className="h-8 px-2">
            <ArrowLeft className="w-4 h-4 mr-1" /> Tesis
          </Button>
          ...
          <section className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            <div className="lg:col-span-1 bg-card dark:bg-slate-900 p-6 rounded-xl border border-border dark:border-slate-800 form-shadow flex flex-col">
              <h4 className="text-sm font-bold text-foreground dark:text-slate-100 mb-6 truncate">
                Risk Seviyesi Durum Dağılımı{chartTitleSuffix && <span className="font-normal text-xs ml-1 opacity-70">({chartTitleSuffix})</span>}
              </h4>
            ...
            <div className="lg:col-span-2 bg-card dark:bg-slate-900 p-6 rounded-xl border border-border dark:border-slate-800 form-shadow flex flex-col">
              <h4 className="text-sm font-bold text-foreground dark:text-slate-100 mb-4 truncate">
                Alt Dağılım{chartTitleSuffix && <span className="font-normal text-xs ml-1 opacity-70">({chartTitleSuffix})</span>}
              </h4>
      ...
      <div className="flex flex-col sm:flex-row gap-3 bg-card p-3 rounded-xl border shadow-sm">
        <div className="flex gap-2 flex-wrap items-center">
          <Button size="sm" variant={filterStatus === '' ? 'default' : 'outline'} onClick={() => setFilterStatus('')} className="h-8 text-xs rounded-full">
            Tümü ({levelRisks.length})
          </Button>
          {Object.entries(STATUS_CONFIG).map(([key, cfg]) => {
            const count = levelRisks.filter((r: any) => r.status === key).length;
            return (
              <Button
        ...
        {isLoading ? (
          ...
          <CardContent className="py-20 text-center">
            ...
            <p className="font-medium text-lg text-muted-foreground">
              {filterStatus ? 'Bu filtrede risk bulunamadı' : 'Bu risk seviyesine ait risk yok'}
            </p>
          </CardContent>
        ) : (
