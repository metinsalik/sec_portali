...
import { DepartmentPrintTable } from './DepartmentPrintTable';
...
export function DepartmentPrintModal({ isOpen, onClose, department, inventoryItems }: DepartmentPrintModalProps) {
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-[95vw] w-full max-h-[95vh] flex flex-col p-0 overflow-hidden">
        <DialogHeader className="px-6 py-4 border-b shrink-0 flex flex-row items-center justify-between">
          <div>
            <DialogTitle>Departman Envanter Çıktısı</DialogTitle>
            <p className="text-sm text-muted-foreground mt-1">Yatay (landscape) düzende departman listesi</p>
          </div>
