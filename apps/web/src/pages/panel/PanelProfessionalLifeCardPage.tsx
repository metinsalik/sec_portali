...
const PanelProfessionalLifeCardPage = () => {
  ...
  if (isLoading) {
    return (
      <div className="p-8 space-y-6 animate-pulse">
        <div className="h-20 bg-muted text-muted-foreground rounded-lg" />
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-1 h-64 bg-muted text-muted-foreground rounded-lg" />
          <div className="md:col-span-2 h-64 bg-muted text-muted-foreground rounded-lg" />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-8 max-w-[1400px] mx-auto space-y-8 animate-in fade-in duration-500">
      {/* Header Section */}
      <div className="flex flex-col md:flex-row md:items-center justify-between bg-card text-card-foreground p-6 rounded-lg shadow-sm border border-border gap-6">
        <div className="flex items-center gap-6">
          ...
          <div className="w-px h-8 bg-muted text-muted-foreground hidden md:block" />
          <div className="flex items-center gap-5">
            <div className="w-14 h-14 bg-muted text-slate-500 rounded-xl flex items-center justify-center text-xl font-bold">
              {prof.fullName.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase()}
            </div>
            <div>
              <div className="flex items-center gap-3 mb-1">
                <h1 className="text-xl font-bold tracking-tight text-foreground">{prof.fullName}</h1>
                {!prof.isActive && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          {/* Certificate Card */}
          <Card className="border-none shadow-sm rounded-lg bg-card text-card-foreground overflow-hidden">
            ...
            <CardContent className="p-6 space-y-5">
              ...
              <div className={cn(
                ...
                prof.certificateStatus.isExpired ? "bg-rose-50/50 border-rose-100 text-destructive" :
                ...
                <div className={cn(
                  ...
                  prof.certificateStatus.isExpired ? "bg-destructive text-destructive-foreground text-white" :
          ...
          <Card className="border-none shadow-sm rounded-lg bg-card text-card-foreground overflow-hidden">
            ...
            <CardContent className="p-6 space-y-4">
              <div className="flex items-center gap-4 group">
                <div className="w-9 h-9 rounded-lg bg-muted flex items-center justify-center text-slate-400 group-hover:bg-primary/5 group-hover:text-primary transition-colors">
            ...
            <CardContent className="p-6 pt-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {activeAssignments.length > 0 ? activeAssignments.map((as: any) => (
                  <div 
                    key={as.id} 
                    className="p-4 bg-slate-50/50 dark:bg-slate-800/50 rounded-xl border border-border hover:border-primary/20 transition-all cursor-pointer group"
                    onClick={() => navigate(`/panel/facilities/${as.facilityId}`)}
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className="w-9 h-9 bg-card text-card-foreground rounded-lg flex items-center justify-center shadow-sm">
                        <Building2 className="w-4 h-4 text-slate-400" />
                      </div>
                      <ExternalLink className="w-3.5 h-3.5 text-slate-300 opacity-0 group-hover:opacity-100 transition-opacity" />
                    </div>
                    <h3 className="font-bold text-sm text-foreground mb-2 truncate">{as.facility.name}</h3>
                ...
                {prof.activityLogs && prof.activityLogs.length > 0 ? prof.activityLogs.map((log: any, idx: number) => (
                  <div key={idx} className="flex gap-4 p-4 hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors rounded-xl group relative">
                    <div className="relative shrink-0">
                      <div className="w-6 h-6 rounded-full bg-muted flex items-center justify-center z-10 relative border border-slate-100 dark:border-slate-700">
                        <div className="w-1.5 h-1.5 rounded-full bg-primary/40" />
                      </div>
                      {idx < prof.activityLogs.length - 1 && (
                        <div className="absolute top-6 left-3 w-px h-[calc(100%+8px)] bg-muted -z-0" />
                      )}
                    </div>
