...
import { Building2, Plus, ChevronRight, AlertTriangle, ShieldCheck, Clock, Activity, ArrowLeft, RefreshCcw, ArrowRight, Upload , MapPin} from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      {!risksLoading && (
        <section className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-card dark:bg-slate-900 p-6 rounded-xl border border-border dark:border-slate-800 form-shadow text-center">
            <p className="text-sm font-medium text-muted-foreground dark:text-slate-400 mb-2">Toplam Lokasyon</p>
      ...
      {!risksLoading && facilityRisks.length > 0 && (() => {
        ...
        const mudahaleCount = filteredFacilityRisks.filter(r => r.status === 'ILK_MUDAHALE_EDILDI').length;
        const takipCount = filteredFacilityRisks.filter(r => r.status === 'TAKIP_SURECINDE').length;
        const kapaliCount = filteredFacilityRisks.filter(r => r.status === 'KAPATILDI_GUVENLI').length;
        ...
        const donutGradient = total > 0 
          ? `conic-gradient(var(--color-error) 0% ${stop1}%, #f97316 ${stop1}% ${stop2}%, #2563eb ${stop2}% ${stop3}%, #22c55e ${stop3}% 100%)`
      ...
      <section className="mb-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
          <h4 className="text-2xl text-foreground dark:text-slate-100 font-bold">Lokasyonlar</h4>
        ...
        {isLoading ? (
          ...
          <div className="col-span-full py-16 text-center text-muted-foreground dark:text-slate-400 bg-muted/30 dark:bg-slate-800/30 rounded-xl border border-dashed border-border dark:border-slate-700">
            ...
            <p className="font-medium">Lokasyon bulunamadı</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            {/* Her Zaman Gösterilen "Tüm Lokasyonlar" Butonu */}
            <div
            ...
            >
              ...
              <h5 className="text-lg font-bold text-foreground">
                {filteredDepartments.length > 3 ? 'Daha Fazla' : 'Tüm Lokasyonlar'}
              </h5>
              <p className="text-sm text-muted-foreground mt-1">
                {filteredDepartments.length > 3 
                  ? `+${filteredDepartments.length - 3} lokasyon daha var` 
                  : 'Tüm lokasyonları yönet ve listele'}
              </p>
            </div>
          </div>
        )}
