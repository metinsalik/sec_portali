...
const pagesDir = path.join(__dirname, 'frontend/src/pages/bina-turu/ayarlar');

const configs = [
  { name: 'AnaGrupYonetimi', endpoint: 'ana-grup', title: 'Ana Grup', singular: 'Ana Grup' },
  { name: 'DenetlenenAlanYonetimi', endpoint: 'denetlenen-alan', title: 'Denetlenen Alan', singular: 'Denetlenen Alan' },
  { name: 'KategoriYonetimi', endpoint: 'kategori', title: 'Kategori', singular: 'Kategori' },
  { name: 'SorumluBirimYonetimi', endpoint: 'sorumlu-birim', title: 'Sorumlu Birim', singular: 'Sorumlu Birim' }
];

const getCrudTemplate = ({ name, endpoint, title, singular }) => `import React, { useState, useEffect } from 'react';
...
const ${name} = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const res = await api.get(\`/bina-turu/ayarlar/${endpoint}?facilityId=\${facilityId}\`);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editId) {
        await api.put(\`/bina-turu/ayarlar/${endpoint}/\${editId}\`, { ad });
        toast.success('${singular} güncellendi.');
      } else {
        await api.post(\`/bina-turu/ayarlar/${endpoint}\`, { facilityId, ad });
        toast.success('${singular} eklendi.');
      }
      setAd('');
      setEditId(null);
      fetchData();
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await api.delete(\`/bina-turu/ayarlar/${endpoint}/\${id}\`);
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>${title} Yönetimi</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="flex gap-2 mb-6">
          <Input 
            placeholder="${singular} Adı" 
            value={ad} 
...
configs.forEach(config => {
  fs.writeFileSync(path.join(pagesDir, config.name + '.tsx'), getCrudTemplate(config));
});

// SorumluKisiYonetimi needs slightly different template (with birimId)
const sorumluKisiCode = `import React, { useState, useEffect } from 'react';
import api from '@/lib/api';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
...
fs.writeFileSync(path.join(pagesDir, 'SorumluKisiYonetimi.tsx'), sorumluKisiCode);

console.log('CRUD components generated.');
