...
// AYLIK HR VERİLERİ
// ─────────────────────────────────────────────────────────────────────────────
router.get('/hr/:facilityId/monthly', async (req, res) => {
    try {
...
router.post('/hr/:facilityId/monthly', async (req, res) => {
    try {
        const facilityId = String(req.params.facilityId);
        const user = getUser(req);
...
router.put('/hr/:facilityId/monthly/:month', async (req, res) => {
    try {
        const facilityId = String(req.params.facilityId);
...
// KAZA VERİLERİ
// ─────────────────────────────────────────────────────────────────────────────
router.get('/accidents/:facilityId/monthly', async (req, res) => {
    try {
        const facilityId = String(req.params.facilityId);
        const month = req.query.month;
        const where = { facilityId };
...
router.post('/accidents/:facilityId/monthly', async (req, res) => {
    try {
        const facilityId = String(req.params.facilityId);
        const user = getUser(req);
...
router.put('/accidents/:facilityId/monthly/:month', async (req, res) => {
    try {
        const facilityId = String(req.params.facilityId);
        const month = String(req.params.month);
...
router.put('/facilities/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const user = getUser(req);
        const { name, shortName, type, city, district, fullAddress, phone, email, website, commercialTitle, taxOffice, taxNumber, sgkNumber, naceCode, dangerClass, employeeCount, buildings } = req.body;
        // Yetki kontrolü
        // @ts-ignore
        const isAssigned = user.isAdmin || user.isManagement || user.facilities?.includes(id);
        if (!isAssigned) {
