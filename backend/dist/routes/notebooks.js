...
router.post('/:facilityId', upload.single('document'), async (req, res) => {
    const { facilityId } = req.params;
...
router.put('/:facilityId/:pageId', upload.single('document'), async (req, res) => {
...
router.delete('/:facilityId/:pageId', async (req, res) => {
    const { facilityId, pageId } = req.params;
    try {
        const existingPage = await prisma.notebookPage.findUnique({
            where: { id: parseInt(pageId) }
...
router.get('/:facilityId/professionals', async (req, res) => {
    const { facilityId } = req.params;
    try {
