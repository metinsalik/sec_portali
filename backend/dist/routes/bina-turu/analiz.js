...
router.get('/', async (req, res) => {
    ...
    const periodStr = req.query.period;
    if (!facilityId) {
    ...
    try {
        ...
        filteredTurlar.forEach(tur => {
            ...
            tur.turSorulari.forEach(ts => {
                if (!ts.cevap)
                    return; // Sadece cevaplanan sorular hesaba katılır
                ...
                if (!anaGrupMap.has(gAd))
                    anaGrupMap.set(gAd, { ad: gAd, toplam: 0, uygun: 0, uygunDegil: 0 });
                const gData = anaGrupMap.get(gAd);
                gData.toplam++;
                // Alan Map
                const aAd = ts.soru.denetlenenAlan?.ad || 'Belirtilmemiş';
                if (!alanMap.has(aAd))
                    alanMap.set(aAd, { ad: aAd, toplam: 0, uygun: 0, uygunDegil: 0 });
        ...
        const calcYuzde = (uygun, toplam) => toplam > 0 ? (uygun / toplam) * 100 : 0;
