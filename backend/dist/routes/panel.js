...
const katipImportService_1 = require("../services/katipImportService");
...
router.post('/facilities/:id/import-katip', upload.single('file'), async (req, res) => {
    try {
        ...
        const result = await (0, katipImportService_1.processKatipImport)(facilityId, file.buffer, username);
        res.json(result);
    }
    catch (error) {
...
router.get('/assignments/compliance-status', async (req, res) => {
    try {
        ...
        const results = facilities.map((f) => {
            ...
            const computeCountdown = (type, isCompliant, required) => {
                if (isCompliant || !required)
                    return { daysLeft: null, startDate: null };
...
router.get('/reports/missing-assignments/excel', async (req, res) => {
    try {
        const facilities = await prisma.facility.findMany({
            where: { isActive: true },
            include: {
        ...
        for (const f of facilities) {
            ...
            if (compliance.overallCompliant)
                continue;
            const getAssignedProsText = (type) => {
            ...
            const formatMissingDetails = (missingMins, requiredMins, employeeCount) => {
                if (employeeCount === 0 || requiredMins === 0)
                    return `${missingMins} dk eksik`;
        ...
        res.setHeader('Content-Disposition', `attachment; filename="eksik_atamalar_raporu_${(0, date_fns_1.format)(new Date(), 'yyyy-MM-dd')}.xlsx"`);
