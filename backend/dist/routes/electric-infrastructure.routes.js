...
const uploadDir = path_1.default.join(process.cwd(), 'uploads', 'electric-infrastructure');
...
router.get('/stats', async (req, res) => {
    try {
        ...
        const noMaintenanceCount = records.filter(r => r.hasMaintenanceRecord === 'Yok' || !r.lastMaintenanceDate).length;
        const openActionCount = records.filter(r => r.actionStatus === 'Açık' || r.actionStatus === 'Devam Ediyor').length;
...
router.get('/facility-status/:facilityId', async (req, res) => {
    try {
...
router.post('/facility-status/:facilityId/toggle', async (req, res) => {
    try {
        const { facilityId } = req.params;
...
router.get('/executive-dashboard', async (req, res) => {
    try {
        ...
        const { facilityType, completionFilter, riskFilter, selectedCategory, criteriaKey, criteriaValue } = req.query;
        // 1. Fetch all facilities or filter by type (default to all active facilities, focused on Hastane if specified)
        const facilityWhere = { isActive: true };
        ...
        const riskByCategoryMap = {};
        const actionStatusMap = { 'Tamamlandı': 0, 'Devam Ediyor': 0, 'Açık': 0 };
        // Criteria breakdown accumulators
        const criteriaBreakdowns = {
            hasRisk: { 'Var': 0, 'Yok': 0 },
        ...
        const facilityDetails = facilities.map(f => {
            ...
            if (criteriaKey && criteriaValue && criteriaValue !== 'all') {
                matchingRecords = matchingRecords.filter(r => r[criteriaKey] === criteriaValue);
            }
...
router.post('/', async (req, res) => {
    try {
        const { facilityId, equipmentCategory, equipmentCodeName, locationDescription, isInspected, hasRisk, hasMaintenanceRecord, lastMaintenanceDate, thermalControl, overloadHeat, cablesBreakers, cleanlinessVentilation, extinguishingSystem, sealingFireStop, protectionSystem, detectedRisk, suggestedAction, emergencyActionTaken, responsiblePerson, deadlineDate, actionStatus, evidenceNo, photoUrls, inspectorName, inspectionDate, notes } = req.body;
        if (!facilityId || facilityId === 'all') {
...
router.get('/export-excel', async (req, res) => {
    try {
        ...
        const rows = records.map((r, idx) => ({
            ...
            'Fotoğraf / kanıt no': r.evidenceNo || (Array.isArray(r.photoUrls) && r.photoUrls.length > 0 ? `${r.photoUrls.length} adet dosya` : ''),
