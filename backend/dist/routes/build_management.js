...
router.get('/projects/:id/risk-assessment', async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (!assessment)
            return res.status(404).json({ error: 'Risk değerlendirmesi bulunamadı' });
        res.json(assessment);
    }
    catch (error) {
        console.error(error);
...
router.get('/projects/:id/handover-ohs', async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/projects/:id/handover-ohs', async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const infectionHandover = await prisma.buildHandoverInfectionInspection.findFirst({ where: { projectId: id } });
...
router.get('/projects/:id/handover-infection', async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/projects/:id/handover-infection', async (req, res) => {
    try {
        const { id } = req.params;
        ...
        // Otomatik Tamamlandı Kontrolü
        const ohsHandover = await prisma.buildHandoverOHSInspection.findFirst({ where: { projectId: id } });
        if (ohsHandover) {
            await prisma.buildProject.update({
                where: { id: id },
                data: { status: 'Tamamlandı' }
...
router.post('/projects/:id/approvals', async (req, res) => {
    try {
        ...
        const permits = await prisma.buildPermit.findMany({ where: { projectId: id } });
