...
const storage = multer_1.default.diskStorage({
    destination: (req, file, cb) => {
        ...
        const targetDir = path_1.default.join(process.cwd(), 'uploads', 'fire_safety_control', cleanFacId);
...
router.get('/settings/all', auth_1.authMiddleware, async (req, res) => {
    try {
...
router.post('/settings/all', auth_1.authMiddleware, async (req, res) => {
    try {
        const user = req.user;
        const isManager = user.isAdmin || user.isManagement || user.roles?.includes('admin') || user.roles?.includes('management');
        if (!isManager) {
...
router.get('/facilities/:facilityId/latest-items', auth_1.authMiddleware, async (req, res) => {
    try {
...
router.post('/items/:itemId/actions', auth_1.authMiddleware, async (req, res) => {
    try {
...
router.post('/items/save', auth_1.authMiddleware, async (req, res) => {
    try {
...
// Tekil madde sil
router.delete('/items/:itemId', auth_1.authMiddleware, async (req, res) => {
    try {
        const { itemId } = req.params;
...
router.post('/seed/kocaeli', auth_1.authMiddleware, async (req, res) => {
    try {
