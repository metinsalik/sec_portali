...
exports.isgDefterService = {
    ...
    updatePagesBulk: async (ids, data) => {
        const updateData = {};
        if (data.ciltNo !== undefined)
            updateData.ciltNo = parseInt(data.ciltNo);
        if (data.pageNo !== undefined)
    ...
    getDashboardStats: async (facilityId, filters = {}) => {
        ...
        // Helpers
        const matchesMonth = (item) => monthInt === null || item.page.date.getMonth() === monthInt;
        const matchesStatus = (item) => {
            if (!activeStatus)
                return true;
            if (activeStatus === 'Tamamlanmamış')
                return item.status !== 'Tamamlandı' && item.status !== 'İptal Edildi';
            return item.status === activeStatus;
        };
        const matchesRisk = (item) => !activeRisk || item.riskLevel === activeRisk;
        const matchesMainCat = (item) => !activeMainCat || item.mainCategory?.name === activeMainCat;
        const matchesCat = (item) => !activeCat || item.category?.name === activeCat;
        // Fully filtered items (for table and totals)
        const fullyFilteredItems = allItems.filter(i => matchesMonth(i) && matchesStatus(i) && matchesRisk(i) && matchesMainCat(i) && matchesCat(i));
