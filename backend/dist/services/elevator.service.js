...
exports.elevatorService = {
    ...
    generateTemplate: async () => {
        ...
        const fillHiddenSheet = (name, data) => {
            if (data.length === 0)
                return null;
    ...
    importExcel: async (facilityIdFallback, buffer) => {
        const workbook = new exceljs_1.default.Workbook();
        await workbook.xlsx.load(buffer);
        const sheet = workbook.getWorksheet('Şablon') || workbook.worksheets[0];
        if (!sheet)
            throw new Error('Şablon sayfası bulunamadı');
        ...
        await prisma.$transaction(async (tx) => {
            for (const rowVals of rows) {
                ...
                const getVal = (idx, fallbackDash = false) => {
