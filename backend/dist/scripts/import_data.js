...
async function main() {
    ...
    const normalize = (s) => s ? s.trim().toLowerCase().replace(/ı/g, 'i').replace(/İ/g, 'i').replace(/ş/g, 's').replace(/Ş/g, 's').replace(/ğ/g, 'g').replace(/Ğ/g, 'g').replace(/ü/g, 'u').replace(/Ü/g, 'u').replace(/ö/g, 'o').replace(/Ö/g, 'o').replace(/ç/g, 'c').replace(/Ç/g, 'c') : '';
    ...
    const osgbFile = fs_1.default.readFileSync(path_1.default.join(dataPath, 'osgb.csv'));
    const osgbData = iconv_lite_1.default.decode(osgbFile, 'windows-1254');
    const osgbRecords = (0, sync_1.parse)(osgbData, {
        columns: true,
        delimiter: ';',
    ...
    const profFile = fs_1.default.readFileSync(path_1.default.join(dataPath, 'profesyoneller.csv'));
    const profData = iconv_lite_1.default.decode(profFile, 'windows-1254');
    const profRecords = (0, sync_1.parse)(profData, {
        columns: true,
        delimiter: ';',
        skip_empty_lines: true,
        trim: true
    ...
    const facilityFile = fs_1.default.readFileSync(path_1.default.join(dataPath, 'tesisler.csv'));
    const facilityData = iconv_lite_1.default.decode(facilityFile, 'windows-1254');
    const facilityRecords = (0, sync_1.parse)(facilityData, {
        columns: true,
        delimiter: ';',
        skip_empty_lines: true,
        trim: true
    });
    const facilityMap = new Map(); // name -> id
    ...
    const assignFile = fs_1.default.readFileSync(path_1.default.join(dataPath, 'atamalar.csv'));
    const assignData = iconv_lite_1.default.decode(assignFile, 'windows-1254');
    const assignRecords = (0, sync_1.parse)(assignData, {
        columns: true,
        delimiter: ';',
        skip_empty_lines: true,
        trim: true
