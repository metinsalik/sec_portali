...
router.get('/', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    for (const loc of locations) {
      ...
      loc.risks.forEach((r: any) => {
        if (r.status === 'ACIK_TEHLIKE') g.stats.acik++;
        if (r.status === 'ILK_MUDAHALE_EDILDI') g.stats.mudahale++;
        if (r.status === 'TAKIP_SURECINDE') g.stats.takip++;
        if (r.status === 'KAPATILDI_GUVENLI') g.stats.kapali++;
      });
    }
