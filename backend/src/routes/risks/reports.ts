...
router.get('/executive/all-facilities', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const user = req.user;
    ...
    const groupCategories: Record<string, { name: string, total: number, closed: number, critical: number }> = {};
    const groupLevels: Record<string, { name: string, initial: number, final: number }> = {};
    ...
    allRisks.forEach((risk: any) => {
      ...
      // Grup geneli
      if (groupLevels[initLvl]) groupLevels[initLvl].initial += 1;
      if (groupLevels[finLvl]) groupLevels[finLvl].final += 1;

      if (!groupCategories[cat]) {
        groupCategories[cat] = { name: cat, total: 0, closed: 0, critical: 0 };
      }
      groupCategories[cat].total += 1;
      if (st === 'KAPATILDI_GUVENLI') groupCategories[cat].closed += 1;
      if (initLvl === 'Tolere Gösterilmez Risk' || initLvl === 'Yüksek Risk') groupCategories[cat].critical += 1;
      ...
      if (facId && facilityMap[facId]) {
        ...
        if (st === 'ACIK_TEHLIKE') fac.activeHazards += 1;
        else if (st === 'KAPATILDI_GUVENLI') fac.closedRisks += 1;
        else fac.inProgress += 1;
...
router.get('/', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const initialLevelsMap: Record<string, number> = {};
    const finalLevelsMap: Record<string, number> = {};
    ...
    const byDepartmentMap: Record<string, { total: number, critical: number, closed: number, totalScore: number }> = {};
    ...
    const responsibleMap: Record<string, { total: number, closed: number, pending: number }> = {};
