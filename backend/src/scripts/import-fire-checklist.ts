...
async function main() {
  ...
  if (!adminUser) throw new Error("Admin user not found. Can't set createdBy.");
  ...
  const questions: { rowIdx: number, item: any }[] = [];
  let itemOrder = 1;

  for (let rowIdx = 7; rowIdx < data.length; rowIdx += 2) {
    const rowData = data[rowIdx] as any[];
  ...
  for (const hc of hospitalColumns) {
    ...
    for (const q of questions) {
      const rowData = data[q.rowIdx] as any[];
      if (!rowData) continue;

      const evalLabel = rowData[hc.colIndex] as string;
      let numericScore = 0;
