...
function determineTitleClass(sertifikaTipi: string): string {
  const t = (sertifikaTipi || '').toLowerCase();
  if (t.includes('a sınıfı')) return 'A Sınıfı IGU';
  if (t.includes('b sınıfı')) return 'B Sınıfı IGU';
  if (t.includes('c sınıfı')) return 'C Sınıfı IGU';
  if (t.includes('hekim')) return 'İşyeri Hekimi';
  if (t.includes('diğer sağlık') || t.includes('dsp')) return 'DSP';
  return 'Diğer';
}

function determineAssignmentType(sertifikaTipi: string): string {
  const t = (sertifikaTipi || '').toLowerCase();
  if (t.includes('sınıfı')) return 'IGU';
  if (t.includes('hekim')) return 'Hekim';
  if (t.includes('sağlık') || t.includes('dsp')) return 'DSP';
  return 'Diğer';
}

export async function processKatipImport(facilityId: string, fileBuffer: Buffer, username: string) {
