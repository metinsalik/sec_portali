...
async function seedCategories() {
  console.log('Seeding default ISG categories...');
  for (const main of mainCategories) {
    let mainCat = await prisma.isgDefterCategory.findFirst({
      where: { name: main.name, parentId: null, facilityId: null }
    });
    
    if (!mainCat) {
      mainCat = await prisma.isgDefterCategory.create({
        data: { name: main.name, facilityId: null, parentId: null }
      });
      console.log(`Created Main Category: ${main.name}`);
    }
    
    for (const sub of main.categories) {
      let subCat = await prisma.isgDefterCategory.findFirst({
        where: { name: sub, parentId: mainCat.id, facilityId: null }
      });
      
      if (!subCat) {
        await prisma.isgDefterCategory.create({
          data: { name: sub, facilityId: null, parentId: mainCat.id }
        });
        console.log(`  Created Category: ${sub}`);
      }
    }
  }
  console.log('Finished seeding ISG categories.');
}
